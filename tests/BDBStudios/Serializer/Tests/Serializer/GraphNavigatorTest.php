...
class GraphNavigatorTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testNavigatorPassesInstanceOnSerialization()
    {
        ...
        $this->context->expects($this->any())
            ->method('getVisitor')
            ->will($this->returnValue($this->getMock('BDBStudios\Serializer\VisitorInterface')));
    ...
    {
        $this->markTestSkipped('Static expects is no longer supported');
        $object = new SerializableClass;
    ...
    {
        $this->context = $this->getMock('BDBStudios\Serializer\Context');
